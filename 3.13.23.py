...
# 版本号：CHATGPT-3.13.23-SYMBOL-PROFILE-REPORT
#
# 更新日志 (v3.13.23):
# 1. 【标的画像报告】新增只读型标的特性分析报告 generate_symbol_profile_report(context)，基于 PTrade 历史行情和本地 state / PnL / stack 数据，输出趋势、波动、回撤、恢复、均值回归、网格效率等评分。
# 2. 【参数倾向建议】新增趋势主导型、波段网格型、高波动慢恢复型、均衡型四类 profile_type 识别，只输出建议，不自动调参。
# 3. 【盘后报告接入】在盘后报表流程末尾生成 reports/symbol_profile_YYYY-MM-DD.json，避开 14:55/14:56 快速收盘路径和盘中下单路径。
# 4. 【交易逻辑零改动】本版本不修改 VA、网格、止盈、滴灌、守门员、影子棘轮、破锁、隔离仓、下单、撤单、补单、PnL 主逻辑。
#
# 更新日志 (v3.13.22):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.13.23-SYMBOL-PROFILE-REPORT'
...
def _calc_returns(closes):
    n=len(closes)
    out={}
    for win in (20,60,120,250):
        k=f'ret_{win}d'
        if n>win and closes[-win-1]>0:
            out[k]=(closes[-1]/closes[-win-1]-1.0)*100.0
        else:
            out[k]=None
    return out


def _calc_volatility(rets, win):
    if len(rets)<win:
        return None
    return float(np.std(rets[-win:])*np.sqrt(252)*100.0)


def _calc_atr_percent(highs,lows,closes,win=14):
    if len(closes)<win+1:return None
    tr=[]
    for i in range(1,len(closes)):
        tr.append(max(highs[i]-lows[i],abs(highs[i]-closes[i-1]),abs(lows[i]-closes[i-1])))
    if len(tr)<win or closes[-1]<=0:return None
    return float(np.mean(tr[-win:])/closes[-1]*100.0)
...
def _calc_recovery_metrics(closes):
    out={}
    n=len(closes)
    for win in (60,120):
        if n<win:
            out[f'rebound_from_{win}d_low']=None;out[f'days_since_{win}d_low']=None;continue
        sub=closes[-win:];low=min(sub);idx=len(sub)-1-sub[::-1].index(low)
        out[f'rebound_from_{win}d_low']=((closes[-1]/low-1.0)*100.0) if low>0 else None
        out[f'days_since_{win}d_low']=win-1-idx
    return out


def _calc_mean_reversion_metrics(closes):
    out={'big_up_threshold':None,'big_down_threshold':None,'big_up_next5_avg':None,'big_down_next5_avg':None}
    if len(closes)<30:return out
    r=np.diff(closes)/np.array(closes[:-1])
    if len(r)<10:return out
    up=np.percentile(r,90);down=np.percentile(r,10)
    upn=[];downn=[]
    for i,x in enumerate(r):
        if i+5>=len(r):break
        nxt=(closes[i+6]/closes[i+1]-1.0)*100.0
        if x>=up:upn.append(nxt)
        if x<=down:downn.append(nxt)
    out['big_up_threshold']=float(up*100.0);out['big_down_threshold']=float(down*100.0)
    out['big_up_next5_avg']=float(np.mean(upn)) if upn else None
    out['big_down_next5_avg']=float(np.mean(downn)) if downn else None
    return out


def _calc_score_from_metrics(m):
    trend=_clamp_score(50 + (_safe_float(m.get('ret_120d'),0)*0.25)+(_safe_float(m.get('ma20_slope'),0)*6)+(_safe_float(m.get('near_120d_high'),0)*0.2))
    vol=_clamp_score(40 + (_safe_float(m.get('vol_20d'),0)*1.2)+(_safe_float(m.get('atr14_pct'),0)*2))
    dd=_clamp_score((_safe_float(m.get('mdd_60d'),0)*0.6)+(_safe_float(m.get('mdd_120d'),0)*0.4))
    recovery=_clamp_score(40 + (_safe_float(m.get('rebound_from_60d_low'),0)*0.8)-(_safe_float(m.get('days_since_60d_low'),0)*0.4))
    mr=_clamp_score(50 + (-_safe_float(m.get('big_up_next5_avg'),0)*2.5)+(_safe_float(m.get('big_down_next5_avg'),0)*2.5))
    ge=None
    if m.get('buy_stack_count') is not None and m.get('sell_stack_count') is not None and m.get('history_pnl') is not None:
        ge=_clamp_score(50+min(25,m['buy_stack_count']+m['sell_stack_count'])+max(-35,min(35,m['history_pnl']/2000.0)))
    return {'trend_score':trend,'volatility_score':vol,'drawdown_score':dd,'recovery_score':recovery,'mean_reversion_score':mr,'grid_efficiency_score':ge}


def _build_symbol_parameter_suggestion(scores):
    sug={'take_profit_threshold':'normal','drawdown_confirm':'normal','tier1_sell_ratio':'normal','drip_weeks':'normal','grid_activity':'normal','cash_buffer':'normal'}
    t=scores.get('trend_score') or 0;r=scores.get('recovery_score') or 0;mr=scores.get('mean_reversion_score') or 0
    v=scores.get('volatility_score') or 0;dd=scores.get('drawdown_score') or 0;ge=scores.get('grid_efficiency_score')
    pt='balanced';exp='特征均衡，建议维持当前参数节奏。'
    if t>=75 and r>=65 and mr<60:
        pt='trend_dominant';sug.update({'take_profit_threshold':'higher','drawdown_confirm':'wider','tier1_sell_ratio':'lower','drip_weeks':'shorter','grid_activity':'conservative'});exp='趋势强且修复快，止盈阈值可提高，降低过早卖出概率。'
    elif mr>=65 and ge is not None and ge>=60:
        pt='grid_swing';sug.update({'grid_activity':'active','tier1_sell_ratio':'higher'});exp='均值回归与网格效率较好，适合更积极的波段网格。'
    elif v>=70 and dd>=70 and r<50:
        pt='high_vol_slow_recovery';sug.update({'take_profit_threshold':'lower','drawdown_confirm':'tighter','tier1_sell_ratio':'higher','drip_weeks':'longer','grid_activity':'conservative','cash_buffer':'higher'});exp='高波动且恢复慢，建议提高现金缓冲并放慢回灌。'
    return pt,sug,exp


def generate_symbol_profile_report(context):
    try:
        reports_dir = research_path('reports');reports_dir.mkdir(parents=True, exist_ok=True)
        date_str = context.current_dt.strftime('%Y-%m-%d')
        payload={'version':__version__,'base_file':'3.13.22.py','date':date_str,'generated_at':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'note':'analysis only; no trading parameter changed','symbols':{}}
        for symbol in getattr(context,'symbol_list',[]):
            try:
