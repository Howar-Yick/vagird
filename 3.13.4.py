...
#
# 更新日志 (v3.13.4):
# 1. 【架构升级】确立 state 文件在初始化时的绝对权威。系统优先从本地持久化文件加载 initial_base_position，彻底解决 symbols.json 中的初始配置在重启时覆盖实盘止盈接力成果的 Bug。
# 2. 【逻辑拨乱反正】止盈触发后直接同步更新 state 中的 initial_base_position，不再依赖额外的市值辅助变量，确立“事实底仓”名分，VA 引擎从此实现全自动化平滑接力。
#
# 更新日志 (v3.13.3):
...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.4] [Func Ver: 4.5]
    [Change]: 确立 state 文件的绝对权威。优先从本地加载 initial_base_position，防止 symbols.json 覆盖实盘状态。
    """
    ...
    # 2. 【核心纠偏】：优先取 state 里的“事实底仓”，如果没有才取 symbols 里的“初始配置”
    ...
    st.update({
        ...
        'initial_position_value': actual_initial_base * cfg['base_price'],
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.13.4] [Func Ver: 6.2]
    [Change]: 止盈后同步更新 initial_base_position 为新底仓，从源头赋予其“名分”，防止误触发释放逻辑。
    """
    try:
        ...
        if len(state.get('trade_week_set', set())) < min_weeks and (pos.amount * price) < min_val: return
        ...
        if tier > 0 and (hwm - profit_ratio) >= {1: 3.0*atr, 2: 5.0*atr, 3: 8.0*atr}.get(tier, 0.05):
            ...
            if sell_amount > 0:
                ...
                if eid:
                    ...
                    # 算出新底仓并同步给 initial 变量，实现逻辑地基的“自我进化”
                    ...
                    state['initial_base_position'] = new_base  # <--- 🌟 修改这里，赋予合法名分
