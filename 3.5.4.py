...
# 版本号：GEMINI-3.5.4-Release
#
# 更新日志 (v3.5.4):
# 1. 【语法修复】彻底重写 reload_config_if_changed，修复 v3.5.3 中非法的表达式串联语法，确保标的增删逻辑可编译。
# 2. 【兼容增强】升级 get_order_status，使用 OrderUtils.normalize 兼容字典与对象返回，提高订单状态查询的鲁棒性。
# 3. 【内存闭环】在热重载删除标的分支中，同步清理 should_place_order_map，防止无效数据残留。
# 4. 【逻辑继承】保留全双工堆栈、信用额度、ATR动态间距及巡检对齐等所有已验证功能。
...
LOG_FH = None
LOG_DATE = None
__version__ = 'GEMINI-3.5.4-Release'
...
def get_order_status(entrust_no):
    """【修正】使用 normalize 增加鲁棒性，应对 PTRADE 将来可能的对象化返回"""
    try:
...
def _apply_price_guard(context, state, buy_p, sell_p, buy_sp, sell_sp):
    """
...
def calculate_atr(context, symbol, atr_period=14):
    ...
    if used_rate is not None: return used_rate
    return None
...
def log_trade_details(context, symbol, trade):
    try:
        ...
        with open(trade_log_path, 'a', encoding='utf-8', newline='') as f:
            ...
            direction, base_position = ("BUY" if trade['entrust_bs'] == '1' else "SELL"), context.state[symbol].get('base_position', 0)
            f.write(",".join([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), symbol, direction, str(trade['business_amount']), f"{trade['business_price']:.3f}", str(base_position), trade.get('entrust_no', 'N/A')]) + "\n")
    except: pass
...
def update_daily_reports(context, data):
    ...
    for symbol in context.symbol_list:
        ...
        amount, close_price = position.amount, context.last_valid_price.get(symbol, context.state[symbol]['base_price'])
