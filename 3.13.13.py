...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.13] [Func Ver: 1.4]
    全面剿灭硬读取 (cfg['key']) 带来的 KeyError 风险，全部替换为 cfg.get 软读取和默认兜底。
    """
    ...
    # 🌟 V3.13.13 修复：防止 cfg['initial_base_position'] 漏写导致崩溃，兜底为 0
    ...
    # 🌟 V3.13.13 修复：防止 cfg['base_price'] 漏写导致崩溃，兜底为 1.0
