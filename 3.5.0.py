...
# 版本号：GEMINI-3.5.0-FullDuplex
#
# 更新日志 (v3.5.0):
# 1. 【风控升级】引入“全双工网格 + 信用额度”机制 (Full-Duplex Grid + Credit Limit)。
# 2. 【核心逻辑】新增 Buy/Sell Stack (买卖优先队列)，实现严格的“盈亏配对”管理。
#    - 卖出时优先匹配最低成本买单（Min-Heap），买入时优先匹配最高卖单（Max-Heap）。
# 3. 【守门员机制】在 place_limit_orders 中增加 The Guard 逻辑：
#    - 除非有信用名额 (credit_limit)，否则禁止亏损挂单，强制修正价格为保本价。
# 4. 【数据结构】State 中增加 buy_stack, sell_stack, credit_limit 持久化字段。
# 5. 【基础继承】包含 v3.4.0 的 ATR 升级、StrategyConfig 重构及盘前热重载修复。
...
LOG_FH = None
LOG_DATE = None
__version__ = 'GEMINI-3.5.0-FullDuplex'
...
def save_state(symbol, state):
    ...
    # 【新增】持久化全双工状态：buy_stack, sell_stack, credit_limit
...
def initialize(context):
    ...
    for sym, cfg in context.symbol_config.items():
        ...
        st.update({
            ...
            # 【新增】全双工网格状态
            'buy_stack': saved.get('buy_stack', []),   # 买入价格栈 (Min-Heap)
            'sell_stack': saved.get('sell_stack', []), # 卖出价格栈 (Max-Heap, stored as negative)
            'credit_limit': saved.get('credit_limit', StrategyConfig.CREDIT_LIMIT), # 信用额度
        ...
        # 确保 stack 是堆结构 (从 JSON 加载后可能是普通列表)
...
def place_limit_orders(context, symbol, state, ignore_cooldown=False, bypass_lock=False, ignore_entrust_nos=None):
    ...
    # ----------------- 【The Guard: 全双工风控守门员】 -----------------
    # 在生成最终挂单价之前，检查历史库存，防止亏损配对。
    # 仅修改挂单价格，不修改 Base Price（基准价）。
    
    # 1. 守门员逻辑：买入检查 (防止高位追高接回空单)
    sell_stack = state.get('sell_stack', [])
    if sell_stack:
        # Sell Stack 是大顶堆，但存储为负数。堆顶 -sell_stack[0] 即为最大卖出价。
        max_sell_price = -sell_stack[0]
        # 如果 计划买入价 > 历史最高卖出价，则是“高买低卖”的亏损行为
        if buy_p > max_sell_price:
            # 检查是否有特批名额
            credit = state.get('credit_limit', 0)
            if credit <= 0:
                # 无名额，强制拦截！将买价下压到保本线以下
                corrected_buy_p = max_sell_price - (max_sell_price * buy_sp)
                corrected_buy_p = round(corrected_buy_p, 3)
                
                # 只有当修正后的价格确实比原计划低时才生效 (避免逻辑打架)
                if corrected_buy_p < buy_p:
                    # info('[{}] 🛡️ Guard: 拦截亏损买入 {:.3f} > 卖出均价 {:.3f}，修正为 {:.3f}', dsym(context, symbol), buy_p, max_sell_price, corrected_buy_p)
                    buy_p = corrected_buy_p

    # 2. 守门员逻辑：卖出检查 (防止低位割肉卖出持仓)
    buy_stack = state.get('buy_stack', [])
    if buy_stack:
        # Buy Stack 是小顶堆。堆顶 buy_stack[0] 即为最低买入价。
        min_buy_price = buy_stack[0]
        # 如果 计划卖出价 < 历史最低买入价，则是“低卖高买”的亏损行为
        if sell_p < min_buy_price:
            # 检查是否有特批名额
            credit = state.get('credit_limit', 0)
            if credit <= 0:
                # 无名额，强制拦截！将卖价上抬到保本线以上
                corrected_sell_p = min_buy_price + (min_buy_price * sell_sp)
                corrected_sell_p = round(corrected_sell_p, 3)
                
                # 只有当修正后的价格确实比原计划高时才生效
                if corrected_sell_p > sell_p:
                    # info('[{}] 🛡️ Guard: 拦截亏损卖出 {:.3f} < 买入成本 {:.3f}，修正为 {:.3f}', dsym(context, symbol), sell_p, min_buy_price, corrected_sell_p)
                    sell_p = corrected_sell_p
    # -------------------------------------------------------------------
    ...
    if ratchet_enabled:
        if abs(price / base - 1) <= 0.10:
            ...
            if ratchet_up:
                ...
                # 棘轮触发后，重新过一遍 Guard 逻辑，确保新生成的 buy_p/sell_p 也不违规
                if sell_stack and buy_p > -sell_stack[0] and state.get('credit_limit', 0) <= 0:
                    buy_p = round(-sell_stack[0] * (1 - buy_sp), 3)
                if buy_stack and sell_p < buy_stack[0] and state.get('credit_limit', 0) <= 0:
                    sell_p = round(buy_stack[0] * (1 + sell_sp), 3)
...
def on_order_filled(context, symbol, order):
    ...
    if order.amount > 0: 
        # === 买入成交 ===
        # 尝试去匹配卖出栈 (Sell Stack - Max Heap)
        # 逻辑：如果能买回比之前卖出价更低的货，就是平了空单/接回了筹码，获利了结。
        matched = False
        if sell_stack:
            max_sell_price = -sell_stack[0] # 堆顶是最大值
            
            # 无论盈亏，优先平掉对手盘堆栈 (Close Opposite)
            ...
            if fill_price > max_sell_price:
                ...
                if current_credit > 0:
                    ...
                    info('[{}] 📉 亏损买回 (Used Credit): 买{:.3f} > 卖{:.3f}, 剩余额度: {}', 
                         dsym(context, symbol), fill_price, max_sell_price, state['credit_limit'])
            else:
                pass # 正常盈利配对
        
        if not matched:
            # 没得配对，说明是新开的多头仓位
            heapq.heappush(buy_stack, fill_price) # 推入买入栈 (Min Heap)

    else:
        # === 卖出成交 ===
        # 尝试去匹配买入栈 (Buy Stack - Min Heap)
        # 逻辑：如果能卖出比之前买入价更高的货，就是平了多单，获利了结。
        matched = False
        if buy_stack:
            min_buy_price = buy_stack[0] # 堆顶是最小值
            ...
            if fill_price < min_buy_price:
                ...
                if current_credit > 0:
                    state['credit_limit'] = current_credit - 1
                    info('[{}] 📉 亏损卖出 (Used Credit): 卖{:.3f} < 买{:.3f}, 剩余额度: {}', 
                         dsym(context, symbol), fill_price, min_buy_price, state['credit_limit'])
            else:
                pass # 正常盈利配对
        
        if not matched:
            # 没得配对，说明是新开的空头/减持仓位
            heapq.heappush(sell_stack, -fill_price) # 推入卖出栈 (Max Heap, 存负数)
...
# ---------------- 配置热重载 (Fix:BasePosition & CreditLimit) ----------------
...
def reload_config_if_changed(context):
    try:
        ...
        StrategyConfig.load(context) # 重新加载全局参数
