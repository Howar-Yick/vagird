...
# 版本号：GEMINI-3.12.6
#
# 更新日志 (v3.12.6 自适应回撤防线 & BUG修复):
# 1. 【止盈重构】引入 3/5/8倍 ATR 动态回撤防线 (ATR-Scaled Drawdown Engine):
#    - 痛点：旧版使用固定的绝对百分比（5%, 8%, 10%）作为回撤扳机，无法适配“千股千策”的多资产组合。对低波资产（如黄金/红利，ATR≈1%）防线太宽，导致见顶后利润回吐过多；对高波资产（如纳指/半导体，ATR≈3%）防线太窄，极易在主升浪中期的正常洗盘中被震荡出局（刻舟求剑）。
#    - 解决：彻底废弃固定百分比，改用与标的真实股性（Macro_ATR）深度绑定的乘数防线，实现量化底层的度量衡统一。
#      * 一阶防线 (Tier 1): 触发 10倍 ATR 利润，容忍 3倍 ATR 回撤。过滤短期噪音，破位即落袋 33%。
#      * 二阶防线 (Tier 2): 触发 20倍 ATR 利润，容忍 5倍 ATR 回撤。契合海龟法则宽容度，过滤中期洗盘，破位落袋 50%。
#      * 三阶防线 (Tier 3): 触发 30倍 ATR 利润，容忍 8倍 ATR 回撤。终极疯牛防线，赋予暴涨资产极大的“深蹲起跳”空间，仅在史诗级泡沫破裂时清仓。
#
# 2. 【紧急修复】修复天地锁破锁机制的 ATR 调用残留 (Hotfix for P0 Bug):
#    - 痛点：v3.12.5 拆分双轨 ATR 后，微观网格挂单逻辑（place_limit_orders）中的破锁极值检测遗留了对废弃函数 `calculate_atr` 的调用，导致大波动跳空时触发 NameError 全局崩溃。
#    - 解决：将缺口探测（gap_pct）防守逻辑，严格对接到高敏的 `calculate_grid_atr`，彻底扫清废弃函数依赖，保障实盘连续竞价平稳运行。
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.12.6] [Func Ver: 4.0]
    [Change]: 引入顶级趋势跟踪 3/5/8 倍 ATR 动态回撤防线，实现全资产股性自适应收割。止盈后彻底清零 HWM 与阶梯记忆。
    """
    try:
        pos = get_position(symbol)
        # 1. 前置拦截：无底仓或处于网格浮亏套牢期（成本<0为已收回本金，暂不处理负成本极端情况）
        if pos.amount == 0 or pos.cost_basis <= 0:
            return

        # 获取标的专属参数
        config = getattr(context, 'symbol_config', {}).get(symbol, {})
        tp_cool_weeks = config.get('tp_cool_weeks', 4)
        min_weeks = config.get('tp_min_weeks', 12)
        min_val = config.get('tp_min_value', 30000)

        # 2. 绝对冷却锁拦截 (防止刚建仓或刚止盈后立刻频繁交易)
        trade_weeks = state.get('trade_week_set', set())
        current_weeks = len(trade_weeks)
        if current_weeks < tp_cool_weeks:
            return
            
        # 3. 幼苗保护 / 长持信仰锁 (时间未达标 且 市值未达标，直接放行不计算)
        pos_value = pos.amount * price
        if current_weeks < min_weeks and pos_value < min_val:
            return

        # 4. 惰性计算宏观 ATR (60日长周期截尾)
        atr = calculate_macro_atr(context, symbol, atr_period=60)
        if atr is None or math.isnan(atr) or atr <= 0:
            atr = 0.02
        state['macro_atr_rate'] = atr  # 存入 state 供看板读取显示
        ...
        # =======================================================
        # 🌟 V4.0 核心引擎：度量衡统一的 ATR 动态防线
        # =======================================================
        # 晋升门槛：赚足 10倍 / 20倍 / 30倍 波动率
        tp_thresholds = {1: 10.0 * atr, 2: 20.0 * atr, 3: 30.0 * atr}
        
        # 动态回撤宽容度：3倍 / 5倍 / 8倍 波动率 (彻底抛弃固定百分比)
        drawdown_limits = {1: 3.0 * atr, 2: 5.0 * atr, 3: 8.0 * atr}
        
        # 卖出比例：1阶切33%，2阶切50%，3阶清仓100%
        sell_ratios = {1: 0.33, 2: 0.50, 3: 1.0}

        # 7. 单向棘轮升级逻辑 (只能升不能降)
        new_tier = current_tier
        for t in [3, 2, 1]:
            if profit_ratio >= tp_thresholds[t]:
                if t > current_tier:
                    new_tier = t
                break
        
        if new_tier > current_tier:
            state['_tp_tier'] = new_tier
            current_tier = new_tier
            if StrategyConfig.DEBUG.ENABLE:
                info('[{}] 🚀 宏观止盈警报升级: Tier {} (当前利润: {:.2%}, ATR基准: {:.2%})', 
                     dsym(context, symbol), current_tier, profit_ratio, atr)

        # 8. 扳机判定与火力覆盖
        if current_tier > 0:
            drawdown = hwm - profit_ratio
            limit = drawdown_limits[current_tier]
            
            if drawdown >= limit:
                sell_ratio = sell_ratios[current_tier]
                sell_amount = math.floor(pos.amount * sell_ratio / 100) * 100
                
                if sell_amount > 0:
                    info('[{}] 💥 触发自适应宏观止盈! 级别:Tier {} (历史最高:{:.2%} -> 现:{:.2%})', 
                         dsym(context, symbol), current_tier, hwm, profit_ratio)
                    info('[{}] 📉 回撤幅度: {:.2%} >= 容忍极限: {:.2%} ({}倍ATR), 下达卖出指令!', 
                         dsym(context, symbol), drawdown, limit, [0,3,5,8][current_tier])
                    
                    order(symbol, -sell_amount, price)
                    
                    # ---------------------------------------------------
                    # 💧 盈余滴灌引擎 (复利飞轮)
                    # ---------------------------------------------------
                    surplus_cash = sell_amount * (price - pos.cost_basis)
                    if surplus_cash > 0:
                        drip_weeks = max(10, min(100, int(0.5 / atr))) 
                        drip_amt = surplus_cash / drip_weeks
                        state['dingtou_base'] += drip_amt
                        info('[{}] 💧 盈余滴灌启动: 变现利润 {:.0f}元, 均摊 {}周, 扩充定投口径 {:.0f}元/周', 
                             dsym(context, symbol), surplus_cash, drip_weeks, drip_amt)
                    
                    # ---------------------------------------------------
                    # 🧠 斩断记忆，物理冷却 (从终结走向新生)
                    # ---------------------------------------------------
                    state['trade_week_set'] = set()  # 清空定投周数，触发 tp_cool_weeks 物理锁
                    state['_tp_hwm_ratio'] = 0.0     # 最高水位线归零
                    state['_tp_tier'] = 0            # 阶梯警报彻底解除
                    
                    # 重新锚定剩余底仓价值，防止 VA 引擎暴走倒灌
                    state['initial_position_value'] = (pos.amount - sell_amount) * price 
                    info('[{}] ♻️ 斩断定投记忆，HWM清零，系统进入 {} 周物理静默冷却期。', 
                         dsym(context, symbol), tp_cool_weeks)
...
# ---------------- 监控与报表生成 (V4.0 HUD 智能指挥中心) ----------------

def generate_html_report(context):
    """
    [Global Ver: v3.12.6] [Func Ver: 5.2 (HUD Pro)]
    [Change]: 在不改变列宽和布局的前提下，采用“主副文本”UI设计，新增显示定投周数、网格单位、总仓/底仓明细。
    """
    try:
        ...
        for symbol in context.symbol_list:
            ...
            if current_weeks < tp_cool_weeks and min_weeks < 999:
                ...
                radar_html = f'<span class="text-dim">静默断代 (剩余 {tp_cool_weeks - current_weeks} 周)</span>'
            elif min_weeks >= 999:
                status_html = '<span class="badge badge-safe">🟢 信仰长拿 (免死)</span>'
                radar_html = '<span class="text-dim">🔒 防线关闭 (无限网格)</span>'
                ...
                radar_html = f'<div class="progress-bg"><div class="progress-fill fill-seed" style="width: {progress}%;"></div></div><div class="text-dim" style="margin-top:4px;">养肥中 (第{current_weeks}/{min_weeks}周)</div>'
            elif tier > 0:
                status_html = f'<span class="badge badge-alert">🔥 Tier {tier} 警戒!</span>'
                drawdown = hwm - profit_ratio
                drawdown_limits = {1: 3.0 * atr, 2: 5.0 * atr, 3: 8.0 * atr}
                limit = drawdown_limits.get(tier, 0.05)
                if limit <= 0: limit = 0.05
                risk_pct = min(100, max(0, int((drawdown / limit) * 100)))
                radar_html = f'<div class="progress-bg"><div class="progress-fill fill-alert" style="width: {risk_pct}%;"></div></div><div class="text-alert" style="margin-top:4px;">距回撤防线 {(limit - drawdown)*100:.1f}%</div>'
            else:
                ...
                radar_html = f'<div class="progress-bg"><div class="progress-fill fill-safe" style="width: {dist_pct}%;"></div></div><div class="text-dim" style="margin-top:4px;">距触发一阶 {(tp_threshold - profit_ratio)*100:.1f}%</div>'

            # --- 3. 弹药健康度 (Ammo) 与 附加数据明细 ---
            ...
            # [新增] 提取所需的附加数据
            int_pos_amt = int(pos_amt)
            base_pos = int(state.get('base_position', 0))
            grid_unit = int(state.get('grid_unit', 0))
            
            # [修复] 增加 white-space: nowrap 强制不换行，稍微压缩进度条和文本间距
            ammo_html = f'<div style="margin-bottom:4px; white-space:nowrap;"><div class="progress-bg" style="width:50px; display:inline-block; vertical-align:middle; margin-right:5px;"><div class="progress-fill {ammo_class}" style="width: {ammo_pct}%;"></div></div><span style="color:#9aa5ce; font-size:12px;">{ammo_pct}%({ammo_text})</span></div><div style="color:#9aa5ce; font-size:11px; white-space:nowrap;">持仓:{int_pos_amt}/底仓:{base_pos}</div>'

            # --- 4. 组装数据并分类 ---
            grid_atr = state.get('grid_atr_rate')
            if isinstance(grid_atr, (int, float)) and not math.isnan(grid_atr) and grid_atr > 0:
                grid_atr_disp = f"{grid_atr*100:.2f}%"
            else:
                grid_atr_disp = "N/A"
            ...
            if min_weeks >= 999:
                ...
                if isinstance(macro_val, (int, float)) and not math.isnan(macro_val) and macro_val > 0:
                    macro_atr_disp = f"{macro_val*100:.2f}%"
                else:
                    macro_atr_disp = "N/A"

            # [新增] 重新拼接 Symbol 的 HTML，加入次级文本 (周数/网格)
            symbol_name = dsym(context, symbol, style='long')
            symbol_html = f"<b>{symbol_name}</b><br><span style=\"color:#9aa5ce; font-size:12px; font-weight:normal;\">定投: {current_weeks}周 | 网格: {grid_unit}股</span>"
            ...
            item = {
                ...
                "pnl_info": f"<span class=\"{'text-safe' if unrealized_pnl>=0 else 'text-alert'}\">{unrealized_pnl:,.2f} <br> <b>{(profit_ratio*100):.2f}%</b></span>",
        ...
        def render_table(items):
            ...
            rows = ""
            for m in items:
                rows += f"<tr><td>{m['symbol_html']}</td><td>{m['status']}</td><td>{m['ammo']}</td><td>{m['price_info']}</td><td>{m['pnl_info']}</td><td>{m['atr_info']}</td><td>{m['radar']}</td></tr>"
            return rows
        ...
        research_path('reports', 'strategy_dashboard.html').write_text(
            html_template.format(
                ...
                g1_rows=render_table(all_metrics['group1']),
                g2_rows=render_table(all_metrics['group2']),
                g3_rows=render_table(all_metrics['group3'])
            ), encoding='utf-8'
        )
    except Exception as e:
