...
# 版本号：CHATGPT-3.13.22.9-MACRO-TP-PENDING-SAFE-CLEANUP
#
# 更新日志 (v3.13.22.9):
# 1. 【pending 下单总闸】补齐 place_limit_orders 的 pending 总闸，阻断所有普通网格下单通道。
# 2. 【启动清理保护】after_initialize_cleanup 跳过 pending 宏观止盈标的挂单，避免误撤导致 pending 卡死。
#
# 更新日志 (v3.13.22.8):
# 1. 【pending 最终总闸】补齐 place_limit_orders 顶部 pending 总闸，禁止 pending 期间任何普通网格下单。
# 2. 【补录保活】pending 期间恢复 fill_recover/patrol 的补录通道，避免 pending 卡住。
#
# 更新日志 (v3.13.22.7):
# 1. 【pending 保护修复】修复 tuple key 误写，并补齐 pending 期间普通网格挂单全通道拦截。
# 2. 【成交识别收紧】收紧宏观止盈成交识别与补录更新，避免 pending 卡住或误认。
#
# 更新日志 (v3.13.22.6):
# 1. 【成交确认两阶段】宏观止盈委托成功后仅登记 pending，待成交回报确认后再正式重锚与启动滴灌。
# 2. 【防误重入】pending 未完成期间阻止重复触发宏观止盈，并增加超时提示日志。
#
# 更新日志 (v3.13.22.5):
# 1. 【作用域修复】修复 handle_data 中 macro_tp_symbols 未定义导致的 NameError。
# 2. 【静默窗补齐】check_pending_rehangs 增加 _ignore_place_until 判断，避免静默窗内提前消耗补单时机。
#
# 更新日志 (v3.13.22.4):
# 1. 【当轮挂单隔离】宏观止盈触发后，当轮跳过普通网格挂单/补偿巡检，避免重锚与未完成成交混用。
# 2. 【下单保护补齐】宏观止盈下单成功后同步 _fill_tracker、pending_frozen 与短暂 _ignore_place_until 静默窗。
#
# 更新日志 (v3.13.22.3):
# 1. 【同轮防回写】宏观止盈重锚完成后，当轮跳过 VA 与 adjust_grid_unit，防止 final_water 被同轮改写。
# 2. 【返回值显式化】_check_macro_take_profit 明确返回 True/False，供 handle_data 做当轮跳过控制。
# 3. 【注释口径修正】统一注释为“宏观止盈卖单允许进入 stack 记账”，避免旧文案误导。
#
# 更新日志 (v3.13.22.2):
# 1. 【重锚精度修复】修复宏观止盈后先按旧 grid_unit 算底仓、再被 adjust_grid_unit 改写导致最终水位偏离目标的问题。
# 2. 【同口径联算】新增 _calc_grid_unit_for_base，并在重锚时联立 base_position 与 grid_unit 选择最接近目标水位的组合。
# 3. 【防回写破坏】宏观止盈重锚后不再无条件调用 adjust_grid_unit，避免刚重锚完成就被再次改坏。
# 4. 【可观测性增强】重锚日志新增 final_water 输出，明确止盈后最终网格水位。
#
# 更新日志 (v3.13.22.1):
# 1. 【宏观止盈后真实重锚】宏观止盈触发后，不再用“剩余持仓 - 最大网格数 × grid_unit”作为新底仓，而是按目标网格水位重新计算真实 base_position。
# 2. 【止盈后中水区】新增宏观止盈后 target_grid_count 逻辑，默认把止盈后的剩余持仓放在 6/10 网格水位，避免止盈后因底仓过低进入深水偏卖。
# 3. 【VA 新起点】止盈后同步更新 base_position、initial_base_position、last_week_position、initial_position_value、max_position，让后续 VA 从新底仓继续计算。
# 4. 【保留 stack 记账】宏观止盈卖单允许进入 sell_stack，不再要求跳过 process_trade_logic，以便后续真实记录买卖盈亏，并继续参与破锁、融合、隔离。
# 5. 【交易逻辑最小改动】不修改普通网格、VA 周期、止盈阈值、滴灌周期、守门员、影子棘轮、破锁、隔离、下单、撤单、补单主逻辑。
#
# 更新日志 (v3.13.22):
...
#
# 更新日志 (v3.13.20):
# 1. 【天地锁单笔隔离】修复天地锁触发时，某方向 stack 只剩单笔记录就被直接删除/移交的问题。现在改为：单笔极值 stack 在天地锁时先转入隔离区，不再粗暴删除，保留历史经济记忆。
# 2. 【隔离区单锚点】新增单锚点隔离机制。隔离区每个方向只保留一个压缩锚点，后续新的单笔极值进入隔离区时，直接与已有隔离锚点融合，避免隔离区膨胀。
# 3. 【有条件融合回归】当主 stack 该方向恢复到至少 3 笔，且当前不处于天地锁处理中时，允许将隔离锚点以“最近价融合”的方式回归主 stack，避免历史极值永久丢失，同时不重新锁死主战场。
#
# 更新日志 (v3.13.19):
# 1. 【半点补单去重】修复 rehang 补单与 patrol 半点巡检窗口重叠时，可能对同一标的重复补单的问题。现在在 rehang 补单后加入短暂 patrol 避让窗口，避免柜台回显延迟导致的重复挂单。
# 2. 【巡检查询瘦身】修复 patrol_and_correct_orders 中冻结量重复查询问题，改为直接复用当前 open_orders 本地计算 pending_frozen，减少不必要的柜台查询压力。
#
# 更新日志 (v3.13.18):
# 1. 【VA特权收紧】修复 VA 建仓特权触发条件过宽的问题。以前进入低水区/浅水区就可能放行买入，导致在实际持仓仍高于底仓时提前追价回补，进而把网格正价差做成负价差。现在改为：只有当实际持仓低于底仓时，才允许 VA 建仓特权豁免买入守门员。
#
# 更新日志 (v3.13.17):
# 1. 【查询瘦身】优化 place_limit_orders 中冻结量的获取方式，改为直接复用当前 open_orders 结果本地计算 pending_frozen，减少不必要的重复柜台查询，降低盘中高频调用压力。
#
# 更新日志 (v3.13.16):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.13.22.9-MACRO-TP-PENDING-SAFE-CLEANUP'
...
def _has_pending_macro_tp(state):
    pending = state.get('_macro_tp_pending')
...
def save_state(symbol, state):
    ...
    store_keys = ['symbol', 'base_price', 'grid_unit', 'max_position', 'last_week_position', 'base_position', 
                  ...
                  'dingtou_base', 'dingtou_rate', '_tp_hwm_ratio', '_tp_tier', '_macro_sell_ids', 'tp_reanchor_grid_ratio',
                  'tp_cool_weeks', 'tp_min_weeks', 'tp_min_value', 'wm_map', 'wm_pnl',
                  'max_grid_count', '_drip_amount', '_drip_remain_weeks',
                  'archived_buy_anchor', 'archived_sell_anchor', '_pending_ignore_ids', '_macro_tp_pending'] # 🌟 V3.13.22 补丁：持久化撤单后待忽略ID白名单
...
def init_symbol_state(context, sym, cfg):
    ...
    st.update({
        ...
        'tp_reanchor_grid_ratio': saved.get('tp_reanchor_grid_ratio') if saved.get('tp_reanchor_grid_ratio') is not None else cfg.get('tp_reanchor_grid_ratio', 0.6),
        ...
        '_macro_tp_pending': saved.get('_macro_tp_pending') if isinstance(saved.get('_macro_tp_pending'), dict) else None,
...
def audit_initial_consistency(context, symbol):
    ...
    try:
        ...
        if theoretical_stack_shares != current_stack_shares:
            info("⚠️ [{}] 审计异常: 实盘持仓网格部分 {} 股, 但 Stack 记录 {} 股。差额: {}。请结合 sell_stack/止盈重锚状态人工确认。", 
    ...
    try:
        ...
        for o in all_orders:
            ...
            if OrderUtils.is_active(order_info):
                std_sym = order_info['std_symbol']
                st = context.state.get(std_sym) if hasattr(context, 'state') else None
                if st and _has_pending_macro_tp(st):
                    info('[{}] ⏭ 存在宏观止盈 pending，启动清理跳过该标的挂单，请人工确认 pending 委托状态。', dsym(context, std_sym))
                    continue
...
def place_auction_orders(context):
    ...
    for sym in context.symbol_list:
        ...
        state = context.state[sym]
        if _has_pending_macro_tp(state):
            info('[{}] ⏭ 宏观止盈 pending 未完成，集合竞价跳过普通挂单。', dsym(context, sym))
            continue
...
def check_pending_rehangs(context):
    ...
    for sym in context.symbol_list:
        ...
        state = context.state[sym]
        if _has_pending_macro_tp(state):
            info('[{}] ⏭ 宏观止盈 pending 未完成，rehang 补单跳过。', dsym(context, sym))
            continue
...
def _is_macro_tp_fill(state, tr, fill_amount, price):
    pending = state.get('_macro_tp_pending')
    ...
    excluded = set(str(x) for x in (pending.get('excluded_entrust_nos') or []))
    if entrust_no and entrust_no in excluded:
        return False

    planned_qty = int(pending.get('planned_qty', 0) or 0)
    filled_qty = int(pending.get('filled_qty', 0) or 0)
    left_qty = max(0, planned_qty - filled_qty)
    this_qty = abs(int(fill_amount))
    if planned_qty <= 0 or this_qty <= 0:
        return False
    if this_qty > left_qty + 200:
        return False

    planned_price = float(pending.get('planned_price', 0.0) or 0.0)
    if planned_price > 0 and price > 0 and abs(price - planned_price) / planned_price > 0.03:
        return False

    if entrust_no and entrust_no == str(pending.get('order_id', '')):
        return True

    fallback_match = False
    created_at = pending.get('created_at')
    try:
        trade_time = tr.get('business_time') or tr.get('time') or tr.get('business_datetime')
        if created_at and trade_time:
            created_dt = datetime.strptime(created_at, '%Y-%m-%d %H:%M:%S')
            if isinstance(trade_time, str) and len(trade_time) >= 8:
                # 兼容 HH:MM:SS
                if len(trade_time) == 8 and ':' in trade_time:
                    tt = datetime.strptime(trade_time, '%H:%M:%S').time()
                    trade_dt = datetime.combine(created_dt.date(), tt)
                else:
                    trade_dt = datetime.strptime(trade_time[:19], '%Y-%m-%d %H:%M:%S')
                fallback_match = (trade_dt >= created_dt)
        else:
            fallback_match = True
    except Exception:
        fallback_match = True
...
def _finalize_macro_tp_pending(context, symbol, state, price):
    pending = state.get('_macro_tp_pending')
    if not isinstance(pending, dict):
        return

    actual_sold_qty = int(pending.get('filled_qty', 0) or 0)
    actual_cash = float(pending.get('filled_cash', 0.0) or 0.0)
    if actual_sold_qty <= 0:
        state['_macro_tp_pending'] = None
        safe_save_state(symbol, state)
        return

    remaining_pos = max(0, int(pending.get('pre_pos', 0) or 0) - actual_sold_qty)
    total_drip_pool = actual_cash + float(pending.get('unreleased_cash', 0.0) or 0.0)
    drip_weeks = max(1, int(pending.get('drip_weeks', 16) or 16))

    state['_drip_amount'] = total_drip_pool / drip_weeks
    state['_drip_remain_weeks'] = drip_weeks
    _reanchor_base_after_macro_tp(context, symbol, state, remaining_pos, price)
    ...
    info('[{}] ✅ 宏观止盈成交确认完成: actual_sold={}, cash={:.2f}, remaining={}, drip={}周，开始正式重锚',
         dsym(context, symbol), actual_sold_qty, actual_cash, remaining_pos, drip_weeks)
    safe_save_state(symbol, state)


def _update_macro_tp_pending_by_amount(context, symbol, state, fill_amount, price, status=None):
    pending = state.get('_macro_tp_pending')
    if not _has_pending_macro_tp(state) or fill_amount >= 0:
        return
    this_qty = abs(int(fill_amount))
    pending['filled_qty'] = int(pending.get('filled_qty', 0) or 0) + this_qty
    pending['filled_cash'] = float(pending.get('filled_cash', 0.0) or 0.0) + this_qty * float(price)

    status = str(status or '')
    planned_qty = int(pending.get('planned_qty', 0) or 0)
    if status == '8' or pending['filled_qty'] >= max(0, planned_qty - 1):
        _finalize_macro_tp_pending(context, symbol, state, price)
    else:
        safe_save_state(symbol, state)


def _update_macro_tp_pending_on_fill(context, symbol, state, tr, fill_amount, price):
    if not _is_macro_tp_fill(state, tr, fill_amount, price):
        return
    status = str(tr.get('status', '') or '')
    _update_macro_tp_pending_by_amount(context, symbol, state, fill_amount, price, status=status)


def on_trade_response(context, trade_list):
    """
    [Global Ver: v3.12.0] [Func Ver: 3.0]
    [Change]: 宏观止盈卖单允许进入 stack 记账，用于真实记录后续买回盈亏，并继续参与破锁、融合、隔离。
    """
    ...
    for tr in trade_list:
        ...
        _update_macro_tp_pending_on_fill(context, sym, state, tr, fill_amount, price)
    ...
    if state.get('_pos_jump_seen_ts') is not None:
        ...
        if not _is_dup_fill(context, key):
            ...
            try:
                ...
                _update_macro_tp_pending_by_amount(context, symbol, state, amount, price, status=None)
            except Exception as e:
...
def patrol_and_correct_orders(context, symbol, state):
    """
    [Global Ver: v3.12.0] [Func Ver: 3.0]
    [Change]: 宏观止盈卖单允许进入 stack 记账，本巡检只负责普通漏单补录和废单清理。
    """
    now_dt = context.current_dt
        ...
        try:
            ...
            for o in all_orders:
                ...
                if delta > 0.9: 
                    ...
                    _update_macro_tp_pending_by_amount(context, symbol, state, real_amount, trade_price, status=None)
...
def _calc_grid_unit_for_base(base_position, price, max_grids):
    scale_multiplier = max(1, int(max_grids) * 2)
    theoretical_unit = int(math.ceil(max(0, base_position) / scale_multiplier / 100.0) * 100)
    if not is_valid_price(price):
        return max(100, theoretical_unit)
...
def _reanchor_base_after_macro_tp(context, symbol, state, remaining_pos, price):
    max_grids = max(2, int(state.get('max_grid_count', 10)))
    ratio = state.get('tp_reanchor_grid_ratio', 0.6)
    try:
        ratio = float(ratio)
    except Exception:
        ratio = 0.6
    ratio = min(0.8, max(0.4, ratio))

    target_grid_count = int(round(max_grids * ratio))
    target_grid_count = max(1, min(max_grids - 1, target_grid_count))

    old_base = int(state.get('base_position', 0) or 0)
    old_unit = max(100, int(state.get('grid_unit', 100) or 100))
    rem = max(0, int(remaining_pos))

    best = None
    for cand_base in range(0, rem + 1, 100):
        cand_unit = _calc_grid_unit_for_base(cand_base, price, max_grids)
        if cand_unit <= 0:
            continue
        bullets = (rem - cand_base) / float(cand_unit)
        score = abs(bullets - target_grid_count)
        candidate = (score, -cand_base, cand_base, cand_unit, bullets)
        if best is None or candidate < best:
            best = candidate

    if best is None:
        chosen_base = max(0, min(rem, int(math.floor((rem - target_grid_count * old_unit) / 100.0) * 100)))
        chosen_unit = old_unit
        final_water = (rem - chosen_base) / float(chosen_unit) if chosen_unit > 0 else 0.0
    else:
        _, _, chosen_base, chosen_unit, final_water = best

    state['base_position'] = chosen_base
    state['initial_base_position'] = chosen_base
    state['last_week_position'] = chosen_base
    state['grid_unit'] = chosen_unit
    state['max_position'] = chosen_base + chosen_unit * max_grids
    state['initial_position_value'] = chosen_base * price

    info('[{}] ♻️ 宏观止盈后重锚: remaining={}, target={}/{}, base {} -> {}, unit {} -> {}, final_water={:.2f}/{}, max_position={}',
         dsym(context, symbol), rem, target_grid_count, max_grids, old_base, chosen_base, old_unit, chosen_unit, final_water, max_grids, state['max_position'])
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    ...
    try:
        ...
        if pos.amount == 0 or pos.cost_basis <= 0: return False
        ...
        if isinstance(pending, dict) and pending.get('status') == 'pending':
            ...
            try:
                if created_at:
                    ...
                    if (datetime.now() - dt_created).total_seconds() >= 300:
                        info('[{}] ⚠️ 宏观止盈 pending 已持续 5 分钟，filled={}/planned={}，请人工关注。', dsym(context, symbol), int(pending.get('filled_qty', 0) or 0), int(pending.get('planned_qty', 0) or 0))
            except Exception:
                pass
            return True
        ...
        if len(state.get('trade_week_set', set())) < tp_cool_weeks: return False
        if len(state.get('trade_week_set', set())) < min_weeks or (pos.amount * price) < min_val: return False
        ...
        if tier > 0 and (hwm - profit_ratio) >= {1: 3.0*atr, 2: 5.0*atr, 3: 8.0*atr}.get(tier, 0.05):
            ...
            if sell_amount > 0:
                ...
                if eid:
                    if '_fill_tracker' not in state:
                        state['_fill_tracker'] = {}
                    state['_fill_tracker'][str(eid)] = 0.0
                    try:
                        context.pending_frozen[symbol] = context.pending_frozen.get(symbol, 0) + abs(sell_amount)
                    except Exception:
                        pass
                    state['_ignore_place_until'] = datetime.now() + timedelta(seconds=60)

                    drip_weeks = {1: 16, 2: 24, 3: 52}.get(tier, 16)
                    state['_macro_tp_pending'] = {
                        'order_id': str(eid),
                        'symbol': symbol,
                        'planned_qty': int(sell_amount),
                        'planned_price': float(price),
                        'tier': int(tier),
                        'drip_weeks': int(drip_weeks),
                        'pre_pos': int(pos.amount),
                        'pre_base_position': int(state.get('base_position', 0)),
                        'pre_grid_unit': int(state.get('grid_unit', 100)),
                        'unreleased_cash': float(state.get('_drip_amount', 0.0) * state.get('_drip_remain_weeks', 0)),
                        'filled_qty': 0,
                        'filled_cash': 0.0,
                        'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                        'status': 'pending',
                        'excluded_entrust_nos': list(cancelled_ids or [])
                    }

                    info('[{}] 🧾 宏观止盈委托已登记待确认: eid={}, planned_qty={}, tier={}', dsym(context, symbol), str(eid), int(sell_amount), int(tier))
                    safe_save_state(symbol, state)
                    return True
    ...
    for sym in context.symbol_list:
        ...
        if is_valid_price(price):
            ...
            if macro_tp_triggered:
                ...
                if _has_pending_macro_tp(st):
                    info('[{}] ⏭ 宏观止盈 pending 未完成，本轮跳过 VA/网格单位调整和普通网格动作。', dsym(context, sym))
                else:
                    info('[{}] ⏭ 宏观止盈处理完成，本轮跳过 VA/网格单位调整和普通网格动作。', dsym(context, sym))
                continue
    ...
    if is_patrol_time:
        for sym in context.symbol_list:
            ...
            if sym in macro_tp_symbols:
                st = context.state[sym]
                if _has_pending_macro_tp(st):
                    patrol_and_correct_orders(context, sym, st)
                log_status(context, sym, st, context.latest_data.get(sym))
                continue
        ...
        for sym in old_symbols.intersection(new_symbols):
            if context.symbol_config[sym] != new_config[sym]:
                ...
                state.update({
                    ...
                    'tp_reanchor_grid_ratio': new_params.get('tp_reanchor_grid_ratio', state.get('tp_reanchor_grid_ratio', 0.6)), 
