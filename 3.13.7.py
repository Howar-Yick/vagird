...
#
# 更新日志 (v3.13.7):
# 1. 【极致减负】微观网格防御上限从 15 档进一步压缩至 12 档，再次强行释放 20% 的沉淀资金。从 20 档到 12 档，总计找回 40% 的流动性，彻底抹除实盘资金压力。
# 2. 【4-4-4 降落伞】重构非对称防御体系，确立更紧凑的“12档 4-4-4 模型”：
#    - 浅水区(0-4档)：买1x / 卖2x（易买难卖，超卖蓄水）
#    - 核心区(4-8档)：买1x / 卖1x（对称咬合，最高频做T）
#    - 深水区(8-12档)：买3x / 卖1x（极难买入，装死防守，稍有反弹即卖出逃生）
#
# 更新日志 (v3.13.6):
# 1. 【风控降维】微观网格防御上限从 20 档精简至 15 档，强行释放 25% 的沉淀资金，大幅缓解极端单边行情下的现金流压力。
# 2. 【智能降落伞】重构网格间距引擎，引入“15档三段式非对称防御体系”：
#    - 浅水区(0-5档)：买1x / 卖2x（易买难卖，超卖蓄水）
#    - 核心区(5-10档)：买1x / 卖1x（对称咬合，最高频做T）
#    - 深水区(10-15档)：买3x / 卖1x（极难买入，稍有反弹即卖出逃生）
#    完美实现“缩小暴跌资金消耗”与“维持日常做T频率”的既要又要。
#
# 更新日志 (v3.13.5):
...
def save_state(symbol, state):
    ...
    # 🌟 紧急修复：加入 buy_grid_spacing 和 sell_grid_spacing
...
def init_symbol_state(context, sym, cfg):
    ...
    st.update({
        ...
        # 🌟 V3.13.7：最大持仓上限降至 12 档
        'max_position': saved.get('max_position', actual_initial_base + saved.get('grid_unit', cfg['grid_unit']) * 12),
...
def _repair_state_logic(context):
    ...
    for sym in context.symbol_list:
        ...
        if current_pos < theoretical_pos * 0.70 and theoretical_pos > state['initial_base_position']:
            ...
            # 🌟 V3.13.7：修复时最高上限同步为 12 档
            state['max_position'] = theoretical_pos + state['grid_unit'] * 12
        ...
        if tier > 0 and (hwm - profit_ratio) >= {1: 3.0*atr, 2: 5.0*atr, 3: 8.0*atr}.get(tier, 0.05):
            ...
            if sell_amount > 0:
                ...
                if eid:
                    ...
                    # 🌟 V3.13.7：止盈后重置最高上限为 12 档
                    state['max_position'] = new_base + unit * 12
...
def update_grid_spacing_final(context, symbol, state, curr_pos):
    ...
    # 🌟 V3.13.7：确立 4-4-4 三段式界限 (0-4, 4-8, 8-12)
    thresh_low, thresh_high = 4, 8
...
def get_target_base_position(context, symbol, state, price, dt):
    try:
        ...
        # 🌟 V3.13.7：VA 加仓后最高上限同步为 12 档
        state['max_position'] = state['base_position'] + state['grid_unit'] * 12
    except Exception: pass
...
def adjust_grid_unit(state):
    """
    [Global Ver: v3.13.7]
    修复参数耦合：保持 25 倍的保守资金缩放比例，但执行 12 档的极限防守纵深。
    """
    # 🌟 保持 25 倍资金缩放压制
    if state['base_position'] > state['grid_unit'] * 25:
        theoretical_unit = math.ceil(state['base_position'] / 25 / 100) * 100
...
def reload_config_if_changed(context):
    """
    [Global Ver: v3.13.7]
    修复热重载对 dingtou_base 的覆盖，保护止盈滴灌的累积成果。
    """
    try:
        ...
        for sym in old_symbols.intersection(new_symbols):
            if context.symbol_config[sym] != new_config[sym]:
                ...
                state.update({
                    ...
                    # 🌟 V3.13.7：修改 JSON 覆盖时，最高上限同步为 12 档
                    'max_position': state['base_position'] + new_params['grid_unit'] * 12
                })
...
def update_daily_reports(context, data):
    ...
    for symbol in context.symbol_list:
        ...
        # 🌟 V3.13.7 核心修复：修正 CSV 数据列为 4档(标准), 8档(中间), 12档(极限)
        row = [
            ...
            str(state['base_position'] + state['grid_unit'] * 4),   # 标准数量 (4档线)
            str(state['base_position'] + state['grid_unit'] * 8),   # 中间数量 (8档线)
            str(state['max_position']),                             # 极限数量 (12档线)
...
def generate_html_report(context):
    """
    [Global Ver: v3.13.7] [Func Ver: 6.4]
    [Change]: 接入水位线重构算法，并在 HUD 大屏精准重构 12 档 (4-4-4) 弹药消耗进度条。
    """
    try:
        ...
        for symbol in context.symbol_list:
            ...
            # 计算当前打出了多少颗“网格子弹” (剔除底仓干扰)
            current_bullets = max(0, (pos_amt - base_pos) / unit) if unit > 0 else 0
            # 进度百分比直接基于 12 颗子弹的总弹夹
            ammo_pct = min(100, int((current_bullets / 12) * 100))
            
            # 颜色预警完美对齐 4-4-4 模型
            if current_bullets <= 4:
                ammo_class, ammo_text = "fill-safe", f"{int(current_bullets)}/12 浅水区"
            elif current_bullets <= 8:
                ammo_class, ammo_text = "fill-alert", f"{int(current_bullets)}/12 核心区"
            else:
                ammo_class, ammo_text = "fill-cooldown", f"{int(current_bullets)}/12 深水警告"
