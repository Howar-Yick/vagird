...
# 版本号：GEMINI-3.5.2-Release
#
# 更新日志 (v3.5.2):
# 1. 【风控闭环】集成全双工网格(Full-Duplex)与信用额度(Credit Limit)机制。
#    - 引入买入栈(Min-Heap)与卖出栈(Max-Heap)管理库存成本。
# 2. 【逻辑修复】提取 _apply_price_guard 公共函数，统一下单与巡检的价格计算逻辑，
#    防止风控挂单被巡检误撤。
# 3. 【配置增强】优化 reload_config_if_changed，支持 credit_limit 热更新及 Config 优先原则。
# 4. 【实盘就绪】包含盘前强制热重载、ATR防抖、PnL计算等所有稳定性修复。
...
LOG_FH = None
LOG_DATE = None
__version__ = 'GEMINI-3.5.2-Release'
...
def initialize(context):
    ...
    for sym, cfg in context.symbol_config.items():
        ...
        st.update({
            ...
            'sell_stack': saved.get('sell_stack', []), # 卖出价格栈 (Max-Heap)
            # 优先级：Config配置 > 历史存档 > 全局默认
...
def before_trading_start(context, data):
    # ------------------ 【新增：盘前强制热重载】 ------------------
    # 确保在 09:15 集合竞价挂单前，内存中的标的列表是最新的
    try:
...
def _apply_price_guard(state, buy_p, sell_p, buy_sp, sell_sp):
    """
    ...
    if sell_stack:
        max_sell_price = -sell_stack[0] # 大顶堆
        if final_buy_p > max_sell_price: # 高买低卖
            credit = state.get('credit_limit', 0)
            if credit <= 0:
                corrected = round(max_sell_price - (max_sell_price * buy_sp), 3)
                if corrected < final_buy_p:
                    info('[{}] 🛡️ 守门员拦截(买): 防止高位接回. 原:{:.3f} 修正:{:.3f} (栈顶卖价:{:.3f})', dsym(context, sym), final_buy_p, corrected, max_sell_price)
                    final_buy_p = corrected

    # 2. 守门员逻辑：卖出检查 (防止低位割肉卖出持仓)
    buy_stack = state.get('buy_stack', [])
    if buy_stack:
        min_buy_price = buy_stack[0] # 小顶堆
        if final_sell_p < min_buy_price: # 低卖高买
            credit = state.get('credit_limit', 0)
            if credit <= 0:
                corrected = round(min_buy_price + (min_buy_price * sell_sp), 3)
                if corrected > final_sell_p:
                    info('[{}] 🛡️ 守门员拦截(卖): 防止低位割肉. 原:{:.3f} 修正:{:.3f} (栈顶买价:{:.3f})', dsym(context, sym), final_sell_p, corrected, min_buy_price)
...
def place_limit_orders(context, symbol, state, ignore_cooldown=False, bypass_lock=False, ignore_entrust_nos=None):
    ...
    # 【修复 v3.5.1】应用公共风控函数，修正价格
    buy_p, sell_p = _apply_price_guard(state, buy_p, sell_p, buy_sp, sell_sp)
...
def patrol_and_correct_orders(context, symbol, state):
    ...
    try:
        ...
        # 【修复 v3.5.1】巡检时同样应用风控逻辑，确保算出来的“正确价格”和下单时一致
        buy_p, sell_p = _apply_price_guard(state, buy_p, sell_p, buy_sp, sell_sp)
...
def reload_config_if_changed(context):
    ...
    包含：标的增删处理、参数热更新、信用额度(credit_limit)即时生效。
    """
    try:
        # 1. 检查文件修改时间
        ...
        # 重新加载全局参数 (如 StrategyConfig 中的配置)
        StrategyConfig.load(context)
        ...
        for sym in new_symbols - old_symbols:
            ...
            st.update({
                ...
                # 回归原始 MaxPos 公式 (不含 scale_factor)
                'max_position': saved.get('max_position', saved.get('base_position', cfg['initial_base_position']) + saved.get('grid_unit', cfg['grid_unit']) * 20),
        ...
        for sym in old_symbols.intersection(new_symbols):
            if context.symbol_config[sym] != new_config[sym]:
                ...
                # 【支持 credit_limit 热更新】
                # 如果配置文件里显式写了 credit_limit，且与当前内存不一致，则更新
