...
# 版本号：CHATGPT-3.14.9-INTRADAY-EDGE-PRESSURE-GRID
# [v3.14.9 更新]
# 1. 新增日内单边吞仓压力控制。
# 2. 根据当前水位、日内起始水位、日内买卖格数、风险侧净推进程度，动态调整风险侧网格间距。
# 3. 中间水位和健康做T场景保持接近原网格间距。
# 4. 只对继续推向 0 或 10 边缘的风险侧加大间距，回归中间方向不刹车。
# 5. 不修改 stack 配对、宏观止盈、VA、滴灌、下单函数、FillPatrol、守门员、天地锁破锁、容量裁剪等主逻辑。
#
# [v3.14.8 更新]
