...
#
# 更新日志 (v3.13.3):
# 1. 【自动化闭环】实现止盈锚点的“自我进化”。系统现在会记住止盈后重置的新市值基准（initial_position_value），重启后不再依赖 symbols.json 里的旧数据，彻底解决了底仓被误判为盈余而导致每分钟释放的 Bug。
#
# 更新日志 (v3.13.2):
# 1. 【安全加固】将 initial_base_position 纳入持久化白名单。确保在止盈重置底仓时，即使外部配置文件读取异常，系统仍能依靠本地记忆恢复正确的底仓锚点。
# 
# 更新日志 (v3.13.1):
# 1. 【滴灌重构】止盈回流机制升级。不再仅滴灌微薄的“纯利润”，而是将止盈卖出的“全部现金回报（本金+利润）”全量纳入滴灌池。
# 2. 【周期加固】适配总额滴灌，将 ATR 滴灌常数微调至 0.6，确保巨量资金回补时具备更科学的战线宽度，维持购买力延续性。
#
# 更新日志 (v3.13.0):
# 1. 【逻辑重构】优化宏观止盈后的底仓重置机制。从原先的“归零重启”升级为“平衡位接力”，将剩余持仓自动锚定在“新底仓+10格筹码”水位，彻底杜绝止盈后的单向抛售风险。
# 2. 【定投衔接】确保止盈动作不破坏 VA（价值平均）引擎的连续性，使得下一周期的定投金额能根据新底仓准确定位，实现“落袋”与“再平衡”的无缝切换。
#
# 更新日志 (v3.12.15):
# 1. 【算法革新】引入“同档水位记录法 (State-Space Cost Reconstruction)”引擎。彻底解耦本地 CSV 弱流水依赖，仅通过券商快照（市价、持仓量、综合收益）的同水位线碰撞，即可 100% 无损逆向提纯真实的 LIFO 网格截留利润。
# 2. 【监控升级】重构 HUD 雷达大屏 (generate_html_report)。新增“纯网格截留”与“利润垫降本额”两大硬核指标渲染，完美隔离券商底层均价波动对策略健康度的视觉干扰。
...
class StrategyConfig:
    ...
    def _load_strategy_config(cls, context, force=False):
        ...
        try:
            ...
            info('⚙️ [Config] Strategy统一配置(V3.12.13)已完成全局覆盖加载')
...
def save_state(symbol, state):
    """
    [Global Ver: v3.13.3] [Func Ver: 2.6]
    [Change]: 将 initial_position_value 加入持久化名单，确立止盈后的新名分。
    """
    ids = list(state.get('filled_order_ids', set()))
    state['filled_order_ids'] = set(ids[-StrategyConfig.MAX_SAVED_FILLED_IDS:])
    
    # [V3.13.3] 必须持久化 initial_position_value 以绕过 symbols.json 的 0 基准覆盖
    store_keys = ['symbol', 'base_price', 'grid_unit', 'max_position', 'last_week_position', 'base_position', 
                  'initial_base_position', 'initial_position_value', # <--- 核心新增
...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.3] [Func Ver: 4.1]
    [Change]: initial_position_value 采用“记忆优先”策略，确保止盈接力成果不被重置。
    """
    ...
    # 【核心逻辑升级】：如果 saved 里有值，说明发生过止盈重置，优先认账！
    # 否则才按 symbols.json 计算初始值。
    mem_initial_val = saved.get('initial_position_value')
    if mem_initial_val is not None and mem_initial_val > 0:
        actual_initial_val = float(mem_initial_val)
    else:
        actual_initial_val = cfg['initial_base_position'] * cfg['base_price']
    ...
    st.update({
        ...
        'initial_base_position': saved.get('initial_base_position', cfg['initial_base_position']),
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.13.3] [Func Ver: 6.1]
    [Change]: 确保 initial_position_value 更新后同步触发持久化。
    """
    try:
        ...
        if current_weeks < tp_cool_weeks: return
            
        pos_value = pos.amount * price
        if current_weeks < min_weeks and pos_value < min_val: return

        atr = calculate_macro_atr(context, symbol, atr_period=60)
        if atr is None or math.isnan(atr) or atr <= 0: atr = 0.02
        ...
        if current_tier > 0:
            ...
            if drawdown >= limit:
                ...
                if sell_amount > 0:
                    info('[{}] 💥 触发自适应宏观止盈! 级别:Tier {} (最高:{:.2%} -> 现:{:.2%})', 
                         dsym(context, symbol), current_tier, hwm, profit_ratio)
                    
                    eid = order(symbol, -sell_amount, price)
                    if eid:
                        macro_ids = state.get('_macro_sell_ids', [])
                        macro_ids.append(str(eid))
                        state['_macro_sell_ids'] = macro_ids[-50:]
                    
                    # ---------------------------------------------------
                    # 💧 资金总量回流滴灌引擎
                    # ---------------------------------------------------
                    total_cash_returned = sell_amount * price
                    if total_cash_returned > 0:
                        drip_weeks = max(12, min(48, int(0.6 / atr))) 
                        drip_amt = total_cash_returned / drip_weeks
                        state['dingtou_base'] += drip_amt
                        info('[{}] 💧 总额滴灌启动：回收现金 {:.0f}元, 增加定投口径 {:.0f}元/周', 
                             dsym(context, symbol), total_cash_returned, drip_amt)
                    
                    # ---------------------------------------------------
                    # ♻️ 止盈再平衡：底仓锚点重置 (逻辑地基在此确立)
                    # ---------------------------------------------------
                    remaining_pos = pos.amount - sell_amount
                    unit = state['grid_unit']
                    
                    initial_base = state.get('initial_base_position', config.get('initial_base_position', 0))
                    new_base_pos = max(initial_base, remaining_pos - 10 * unit)
                    new_base_pos = math.floor(new_base_pos / 100) * 100
                    
                    state['base_position'] = new_base_pos
                    state['last_week_position'] = new_base_pos
                    
                    # 🌟 关键：立即更新初始价值记忆，确立新起点，防止被判定为盈余
                    state['initial_position_value'] = new_base_pos * price
                    
                    state['max_position'] = new_base_pos + unit * 20
                    state['trade_week_set'] = set() 
                    state['_tp_hwm_ratio'] = 0.0     
                    state['_tp_tier'] = 0            
                    
                    info('[{}] ♻️ 止盈再平衡完成：剩余持仓 {} 股，新底仓起点 {} 股 (锚点价值: {:.0f}元)。系统进入 {} 周冷却期。', 
                         dsym(context, symbol), remaining_pos, new_base_pos, state['initial_position_value'], tp_cool_weeks)
...
def reload_config_if_changed(context):
    """
    [Global Ver: v3.12.15] [Func Ver: 2.1]
    [Change]: 补齐热重载新增标的时的水位线利润记忆 (wm_map, wm_pnl) 继承。
    """
    try:
        ...
        for sym in new_symbols - old_symbols:
            ...
            st.update({
                ...
                # 🌟 [V3.12.15 终极修复点]: 热加载新增标的时，完美继承水位线数据！
                'wm_map': saved.get('wm_map', {}),
                'wm_pnl': saved.get('wm_pnl', 0.0)
            })
