...
# 版本号：CHATGPT-3.14.3-MACRO-TP-PARTIAL-FILL-SAVE
#
# 更新日志 (v3.14.3):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.14.3-MACRO-TP-PARTIAL-FILL-SAVE'
    ...
    if is_main_trading_time():
        try:
            ...
            for o in all_orders:
                ...
                is_macro_tp_order = _is_current_macro_tp_order_id(state, eid)
...
def _is_current_macro_tp_order_id(state, entrust_no):
    task = _get_macro_tp_task(state)
    if not task:
        return False
    if task.get('status') not in ['pending', 'partial_filled']:
        return False
    en = str(entrust_no or '')
    task_order_id = str(task.get('order_id', '') or '')
    if not en or not task_order_id:
        return False
    if en.startswith('SYN-'):
        return False
    return en == task_order_id
...
def _is_macro_tp_trade(context, symbol, state, tr, fill_amount, price):
    ...
    return _is_current_macro_tp_order_id(state, entrust_no)
