...
# 版本号：CHATGPT-3.13.22.5-MACRO-TP-GUARD-SCOPE-FIX
#
# 更新日志 (v3.13.22.5):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.13.22.5-MACRO-TP-GUARD-SCOPE-FIX'
        ...
        if tier > 0 and (hwm - profit_ratio) >= {1: 3.0*atr, 2: 5.0*atr, 3: 8.0*atr}.get(tier, 0.05):
            ...
            if sell_amount > 0:
                ...
                if eid:
                    ...
                    info('[{}] ♻️ 止盈重置成功：锁定新底仓 {} 股，新增及滚存现金共 {:.2f} 元，将分 {} 周平滑滴灌。', dsym(context, symbol), state.get('base_position', 0), total_drip_pool, drip_weeks)
                    safe_save_state(symbol, state)
                    return True
