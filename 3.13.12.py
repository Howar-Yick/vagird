...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.12] [Func Ver: 1.3]
    增加严格的 NoneType 兜底校验，防止 JSON 中的 null 引发比较崩溃。
    """
    ...
    st.update({
        ...
        # 🌟 V3.13.12 修复 JSON null 解析漏洞，加入 is not None 的严格守门
...
def reload_config_if_changed(context):
    """
    [Global Ver: v3.13.12] [Func Ver: 3.1]
    消灭手工复制的庞大状态生成字典，直接复用 init_symbol_state 保证热加载100%安全。
    新增 dingtou_base 盘中热重载支持。
    """
    try:
        ...
        for sym in new_symbols - old_symbols:
            ...
            cfg = new_config[sym]
            # 🌟 V3.13.12 极致重构：直接调用冷启动相同的初始化函数，杜绝任何参数遗漏！
            init_symbol_state(context, sym, cfg)
        ...
        for sym in old_symbols.intersection(new_symbols):
            if context.symbol_config[sym] != new_config[sym]:
                ...
                state.update({
                    ...
                    'dingtou_base': new_params.get('dingtou_base', state.get('dingtou_base', 0)), # 🌟 V3.13.12 支持热更新定投基数
