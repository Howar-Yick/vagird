...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.2] [Func Ver: 4.0]
    [Change]: 显式记录并持久化 initial_base_position，增强极端配置异常下的生存能力。
    """
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.13.1] [Func Ver: 6.0]
    [Change]: 实现“总额滴灌”逻辑，将卖出总金额除以动态周期（常数0.6）加回定投基数。
    """
    try:
            ...
            if drawdown >= limit:
                ...
                if sell_amount > 0:
                    ...
                    if total_cash_returned > 0:
                        # 使用常数 0.6 适配更大的滴灌总额
                        # 波动率 0.015(1.5%) 时 drip_weeks = 40周
                        # 波动率 0.04(4%) 时 drip_weeks = 15周
                        drip_weeks = max(12, min(48, int(0.6 / atr))) 
                        
                        drip_amt = total_cash_returned / drip_weeks
                        state['dingtou_base'] += drip_amt
                        
                        info('[{}] 💧 总额滴灌启动：回收现金 {:.0f}元, 均摊 {}周 买回, 增加定投口径 {:.0f}元/周', 
                             dsym(context, symbol), total_cash_returned, drip_weeks, drip_amt)
                    
                    # ---------------------------------------------------
                    # ♻️ 止盈再平衡：底仓锚点重置 (保留 v3.13.0 成果)
                    # ---------------------------------------------------
                    remaining_pos = pos.amount - sell_amount
                    unit = state['grid_unit']
                    
                    # 使用 state.get 确保 initial_base_position 的安全性
                    ...
                    info('[{}] ♻️ 止盈再平衡完成：剩余持仓 {} 股，新底仓起点重置为 {} 股。系统进入 {} 周冷却期。', 
                         dsym(context, symbol), remaining_pos, new_base_pos, tp_cool_weeks)
