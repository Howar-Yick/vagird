...
# 版本号：GEMINI-3.5.1-PatrolFix
#
# 更新日志 (v3.5.1):
# 1. 【逻辑修复】提取 _apply_price_guard 公共函数，统一“下单”与“巡检”的价格计算逻辑，
#    彻底解决“风控挂高价单”被“巡检按低价单”误撤的死循环 BUG。
# 2. 【配置优先】Initialize 中 credit_limit 读取逻辑改为优先读取 symbols.json 配置，
#    确保修改配置文件重启后能即时生效。
# 3. 【完整功能】包含全双工网格、ATR动态间距、盘前热重载等所有特性。
...
LOG_FH = None
LOG_DATE = None
__version__ = 'GEMINI-3.5.1-PatrolFix'
...
def initialize(context):
    ...
    for sym, cfg in context.symbol_config.items():
        ...
        st.update({
            ...
            # 【修复 v3.5.1】优先读取 Config 中的 credit_limit，实现重启即生效
            # 优先级：Config > Saved State > Default
    ...
    if sell_stack:
        ...
        if final_buy_p > max_sell_price: # 高买低卖
            ...
            if credit <= 0:
                ...
                if corrected < final_buy_p: final_buy_p = corrected
    ...
    if buy_stack:
        ...
        if final_sell_p < min_buy_price: # 低卖高买
            ...
            if credit <= 0:
                ...
                if corrected > final_sell_p: final_sell_p = corrected
