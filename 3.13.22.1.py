...
# 版本号：CHATGPT-3.13.22.1-MACRO-TP-REANCHOR
#
# 更新日志 (v3.13.22.1):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.13.22.1-MACRO-TP-REANCHOR'
...
def _reanchor_base_after_macro_tp(context, symbol, state, remaining_pos, price):
    unit = max(100, int(state.get('grid_unit', 100)))
    ...
    new_base = int(remaining_pos - target_grid_count * unit)
    new_base = max(0, min(int(remaining_pos), new_base))
    new_base = int(math.floor(new_base / 100.0) * 100)
    new_base = max(0, min(int(remaining_pos), new_base))

    state['base_position'] = new_base
    state['initial_base_position'] = new_base
    state['last_week_position'] = new_base
    state['max_position'] = new_base + unit * max_grids
    state['initial_position_value'] = new_base * price

    info('[{}] ♻️ 宏观止盈后重锚: remaining={}, unit={}, target={}/{}, base {} -> {}, max_position={}',
         dsym(context, symbol), int(remaining_pos), unit, target_grid_count, max_grids, old_base, new_base, state['max_position'])
