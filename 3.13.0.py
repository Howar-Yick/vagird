...
def save_state(symbol, state):
    """
    [Global Ver: v3.12.15] [Func Ver: 2.4]
    [Change]: 将水位线算法账本 (wm_map, wm_pnl) 加入持久化白名单，防止重启失忆。
    """
    ids = list(state.get('filled_order_ids', set()))
    state['filled_order_ids'] = set(ids[-StrategyConfig.MAX_SAVED_FILLED_IDS:])
    
    # [V3.12.5] 引入双轨制ATR + [V3.12.15] 水位线数据持久化
...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.12.15] [Func Ver: 3.5]
    """
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.13.0] [Func Ver: 5.0]
    [Change]: 优化止盈后底仓重置逻辑，从“直接归零”改为“重置于10格平衡位”，确保定投及网格接力。
    """
    try:
            ...
            if drawdown >= limit:
                ...
                if sell_amount > 0:
                    ...
                    if surplus_cash > 0:
                        ...
                        info('[{}] 💧 盈余滴灌: 利润 {:.0f}元, 均摊 {}周, 增加定投口径 {:.0f}元/周', 
                    ...
                    # 将剩余持仓重置在“底仓+10格筹码”的位置，若不足则设为初始底仓
                    new_base_pos = max(state.get('initial_base_position', 0), remaining_pos - 10 * unit)
                    new_base_pos = math.floor(new_base_pos / 100) * 100 # 股数凑整
                    ...
                    info('[{}] ♻️ 止盈再平衡完成：剩余持仓 {} 股，新底仓起点重置为 {} 股 (保留10格网格筹码)。系统进入 {} 周冷却期。', 
