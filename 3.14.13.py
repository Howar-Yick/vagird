...
# 版本号：CHATGPT-3.14.13-EDGE-SPACING-FLOOR
# [v3.14.13 更新]
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.14.13-EDGE-SPACING-FLOOR'
...
def update_grid_spacing_final(context, symbol, state, curr_pos):
    ...
    _ensure_intraday_edge_state(context, symbol, state, pos)
    pressure_ctx = _calc_intraday_edge_pressure(context, symbol, state, pos)
    side = pressure_ctx['side']
    multiplier = pressure_ctx['multiplier']

    buy_mult = multiplier if side == 'buy' else 1.0
    sell_mult = multiplier if side == 'sell' else 1.0

    edge_floor = _calc_edge_spacing_floor(state, pos)
    buy_mult = max(buy_mult, edge_floor['buy_floor_mult'])
    sell_mult = max(sell_mult, edge_floor['sell_floor_mult'])

    state['_edge_block_buy'] = bool(edge_floor['block_buy'])
    state['_edge_block_sell'] = bool(edge_floor['block_sell'])
    state['_edge_buy_floor_mult'] = edge_floor['buy_floor_mult']
    state['_edge_sell_floor_mult'] = edge_floor['sell_floor_mult']
    state['_edge_water_runtime'] = edge_floor['water']

    buy_spacing = base_spacing * buy_mult
    sell_spacing = base_spacing * sell_mult
    ...
    if new_buy != old_buy or new_sell != old_sell:
        ...
        info('[{}] 🌊 单边吞仓压力: water={:.2f} start={:.2f} buyG={:.2f} sellG={:.2f} side={} pressure={:.2f} mult={:.2f} floor=[买{:.2f},卖{:.2f}] block=[buy={},sell={}] -> [买{:.2%},卖{:.2%}]',
             dsym(context, symbol), pressure_ctx['water'], float(pressure_ctx.get('start_water', pressure_ctx['water']) or pressure_ctx['water']),
             pressure_ctx['buy_grids'], pressure_ctx['sell_grids'], side, pressure_ctx['pressure'], multiplier,
             edge_floor['buy_floor_mult'], edge_floor['sell_floor_mult'],
