...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.14] [Func Ver: 1.5]
    终极防御：修复 JSON 中 List/Set/Dict 类型字段为 null 时，引发的 NoneType 不可迭代崩溃。
    """
    ...
    st.update({
        ...
        # 🌟 V3.13.14 核心修复：防止 saved.get 返回 None 导致 set() 解析崩溃，改用 or [] 绝对兜底
    ...
    for key in ['buy_stack', 'sell_stack']:
        # 🌟 V3.13.14 核心修复：防止 NoneType is not iterable
