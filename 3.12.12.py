...
#
# 更新日志 (v3.12.12):
# 1. 【规则对齐】修复宏观止盈 Tier 3 (100% 清仓) 时的碎股残留 Bug。
# 2. 【逻辑重构】在一、二阶减仓时保持向下取整 100 股的安全规则；但在触发终极清仓时，绕过取整逻辑，直接全量发起 `pos.amount` 的卖出委托，确保一次性彻底清理包含尾数（如 1250 股）的账户粉尘。
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.12.12] [Func Ver: 4.1]
    [Change]: 修复 Tier 3 清仓时取整导致碎股残留的问题。
    """
    try:
        ...
        if current_tier > 0:
            ...
            if drawdown >= limit:
                ...
                # ---------------------------------------------------
                # [V3.12.12 修复]: Tier 3 全部清仓时支持碎股卖出，不向下取整
                # ---------------------------------------------------
                if current_tier == 3:
                    sell_amount = pos.amount  # 终极清仓，粉尘不留
                else:
                    sell_amount = math.floor(pos.amount * sell_ratio / 100) * 100  # 局部减仓，遵守百股限制
...
def generate_html_report(context):
    """
    [Global Ver: v3.12.10] [Func Ver: 6.2 (HUD Pro Radar Fix)]
    [Change]: 修复丢失的“现金与其他”雷达条，并尝试穿透获取真实可用资金(available_cash)加入总盘计算。
    """
    try:
        ...
        # --- 6. 全局资产比例雷达计算 ---
        # [V3.12.10 新增] 尝试安全获取真实的账户可用资金，加入“其他”分类
        try:
