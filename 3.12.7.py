...
#
# 更新日志 (v3.12.7):
# 1. 【看板升级】全面升级 HUD Pro 2.0 交互架构，引入“主控行+抽屉”单页折叠设计，解决海量标的信息过载问题。
# 2. 【数据穿透】抽屉面板打通底层网格状态，实时可视化展示 Buy/Sell Stack（套牢阻力与网格支撑）及 VA 引擎运行详情。
# 3. 【资产调度前置】引入全局大类资产（科技/黄金/红利）占比监控雷达，为后续“哈利·布朗永久组合”全局资金动态流转提供直观决策依据。
# 4. 【安全隔离】UI 渲染逻辑完全剥离于交易主线程，HTML 生成异常将被安全捕获，绝不阻断实盘交易。
...
def generate_html_report(context):
    """
    [Global Ver: v3.12.7] [Func Ver: 6.0 (HUD Pro 2.0)]
    [Change]: 升级HUD Pro 2.0，增加哈利布朗资产占比雷达，及单页抽屉式(Drawer)堆栈可视化功能。
    """
    try:
        ...
        for symbol in context.symbol_list:
            ...
            # 引入 onClick 触发抽屉逻辑
            symbol_html = f"<div style=\"cursor:pointer; color:#7aa2f7; font-weight:bold; font-size:14px;\" onclick=\"toggleDrawer('{sym_id_js}')\">🔽 {symbol_name}</div><span style=\"color:#9aa5ce; font-size:11px; margin-left:18px;\">定投: {current_weeks}周 | 网格: {grid_unit}股</span>"

            # --- [核心] 5. 抽屉式面板(Drawer)深度微观透视 ---
            ...
            <td colspan="7" style="padding: 0; border: none;">
                <div id="drawer-{sym_id_js}" style="display: none; background: #1f2335; padding: 12px 15px; margin: 4px 10px 15px 10px; border-left: 3px solid #7aa2f7; border-radius: 4px;">
                    <div style="color: #c0caf5; font-size: 13px; margin-bottom: 6px;"><b>🧱 堆栈微观阵地 (Stack Radar):</b></div>
                    <div style="color: #f7768e; font-size: 12px; margin-left: 15px;">🔴 <b>上方套牢阻力 (Sell Stack):</b> {s_str}</div>
        ...
        if not template_file.exists(): 
            info("⚠️ dashboard_template.html 不存在，请尽快更新！(本次跳过报表生成)")
            return
        ...
        research_path('reports', 'strategy_dashboard.html').write_text(
            html_template.format(
                ...
                portfolio_radar=portfolio_html,
        ...
        log.error(f"⚠️ 生成 HUD 面板异常(防抖生效不阻断主逻辑): {e}")