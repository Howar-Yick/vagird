...
# 版本号：CHATGPT-3.14.1-MACRO-TP-STATE-MACHINE-FINAL-GUARD
#
# 更新日志 (v3.14.1):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.14.1-MACRO-TP-STATE-MACHINE-FINAL-GUARD'
...
def _is_macro_tp_trade(context, symbol, state, tr, fill_amount, price):
    ...
    if not task or task.get('status') not in ['pending','partial_filled'] or fill_amount>=0: return False
    en=str(tr.get('entrust_no',''))
    if en in set(task.get('excluded_entrust_nos',[])): return False
    if en and en==str(task.get('order_id','')): return True
    try:
        ct=datetime.strptime(str(task.get('created_at')),'%Y-%m-%d %H:%M:%S')
    except: ct=context.current_dt
    if context.current_dt < ct: return False
    if abs(float(price)-float(task.get('planned_price',price)))/(abs(float(task.get('planned_price',price)))+1e-9) > 0.03: return False
    remain=max(0,float(task.get('planned_qty',0))-float(task.get('filled_qty',0)))
    if abs(float(fill_amount))>remain+200: return False
    return _has_active_macro_tp_task(state)
