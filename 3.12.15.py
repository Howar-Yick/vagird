...
#
# 更新日志 (v3.12.14):
# 1. 【风控升级】修复守门员函数的同价摩擦漏洞。将边界判定从严格的 < / > 改为 <= / >=，并利用动态 ATR 强制拉开同价位单的利润空间，杜绝零利润白交手续费的无效交易。
# 
# 更新日志 (v3.12.13):
# 1. 【架构重构】重构 StrategyConfig，引入级联覆盖模式，确立 strategy.json 为最高优先级统管文件，兼容旧文件防崩溃。
# 2. 【逻辑重构】修复宏观止盈大单物理隔离失效的漏洞，确保大单抛售时不污染微观网格堆栈。
...
def _check_macro_take_profit(context, symbol, state, price, dt):
    """
    [Global Ver: v3.12.13] [Func Ver: 4.2]
    [Change]: 修复宏观单未记录 eid 导致物理隔离失效的严重 Bug。
    """
    try:
