...
def save_state(symbol, state):
    ...
    store_keys = ['symbol', 'base_price', 'grid_unit', 'max_position', 'last_week_position', 'base_position', 
                  ...
                  'max_grid_count'] # 🌟 V3.13.8 补丁：将其加入硬盘持久化白名单
...
def init_symbol_state(context, sym, cfg):
    """
    [Global Ver: v3.13.9.1] (Hotfix)
    修复冷启动参数覆盖：max_grid_count 作为静态配置，必须绝对服从 symbols.json，无视历史 state 记忆！
    """
    ...
    # 🌟 修复点 1：直接读取 cfg (symbols.json)，不再用 saved.get() 去读历史记忆！
    ...
    st.update({
        ...
        # 🌟 修复点 2：天花板强制使用最新算出的 max_grids，拒绝加载陈旧的计算结果
        ...
        if tier > 0 and (hwm - profit_ratio) >= {1: 3.0*atr, 2: 5.0*atr, 3: 8.0*atr}.get(tier, 0.05):
            ...
            if sell_amount > 0:
                ...
                if eid:
                    ...
                    # 🌟 V3.13.9 核心：事实底仓大幅下降，立即调用液压引擎，强制网格火力缩容！
                    adjust_grid_unit(state)
                    
                    state['trade_week_set'] = set() 
                    state['_tp_hwm_ratio'], state['_tp_tier'] = 0.0, 0
                    
                    info('[{}] ♻️ 止盈重置成功：锁定新底仓 {} 股，释放现金将分 {} 周滴灌接回。', dsym(context, symbol), new_base, drip_weeks)
                    safe_save_state(symbol, state)
