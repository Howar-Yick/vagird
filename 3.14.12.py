...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.14.12-MACRO-TP-HWM-UPDATE-BEFORE-SKIP'
...
def place_limit_orders(context, symbol, state, ignore_cooldown=False, bypass_lock=False, ignore_entrust_nos=None):
    ...
    try:
        ...
        buy_allowed = can_place_buy and (not same_buy) and can_buy_by_position
        ...
        sell_allowed = can_place_sell and can_sell and can_sell_by_enable and can_sell_by_base
        if unlock_rehang_requested:
            if sell_allowed:
...
def update_grid_spacing_final(context, symbol, state, curr_pos):
    ...
    if side == 'buy':
        buy_spacing = base_spacing * multiplier
    elif side == 'sell':
        sell_spacing = base_spacing * multiplier
    ...
    if new_buy != old_buy or new_sell != old_sell:
        ...
        info('[{}] 🌊 单边吞仓压力: water={:.2f} start={:.2f} buyG={:.2f} sellG={:.2f} side={} pressure={:.2f} mult={:.2f} -> [买{:.2%},卖{:.2%}]',
             dsym(context, symbol), pressure_ctx['water'], float(pressure_ctx.get('start_water', pressure_ctx['water']) or pressure_ctx['water']),
             pressure_ctx['buy_grids'], pressure_ctx['sell_grids'], side, pressure_ctx['pressure'], multiplier, new_buy, new_sell)
