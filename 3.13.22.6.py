...
# 版本号：CHATGPT-3.13.22.6-MACRO-TP-FILL-CONFIRM
#
# 更新日志 (v3.13.22.6):
...
LOG_FH = None
LOG_DATE = None
__version__ = 'CHATGPT-3.13.22.6-MACRO-TP-FILL-CONFIRM'
    ...
    if ratchet_enabled:
        if abs(price / base - 1) <= 0.10:
            ...
            if ratchet_up:
                ...
                if cancelled_ids: state['_pending_ignore_ids', '_macro_tp_pending'] = list(cancelled_ids)
...
def _is_macro_tp_fill(state, tr, fill_amount, price):
    ...
    if planned_price > 0 and price > 0:
        if abs(price - planned_price) / planned_price > 0.03:
            return False
    ...
    for sym in context.symbol_list:
        ...
        if is_valid_price(price):
            ...
            if macro_tp_triggered:
                ...
                info('[{}] ⏭ 宏观止盈重锚已完成，本轮跳过 VA/网格单位调整和普通网格挂单。', dsym(context, sym))
                continue
