...
#
# 更新日志 (v3.12.9):
# 1. 【UI 热修复】修复 HUD Pro 2.0 表格列宽挤压与徽章(Badge)错位换行问题。
# 2. 【前端加固】全局引入 td/th 纵向居中与 white-space: nowrap 强制约束，确保状态灯与弹药条视觉上的绝对齐整。
# 3. 【视觉优化】为收割雷达(Radar)增加定宽外包装，防止长名称标的导致进度条被动拉伸。
...
def generate_html_report(context):
    """
    [Global Ver: v3.12.9] [Func Ver: 6.1 (HUD Pro Fix)]
    [Change]: 修复UI渲染挤压问题，为进度条添加强制宽度包装 (wrapper)，适配无换行表格。
    """
    try:
        ...
        for symbol in context.symbol_list:
            ...
            # --- [核心修复] Status & Radar：添加 110px 包装宽度防止挤压 ---
            status_html = ""
            radar_html = ""
            ...
            # --- [核心修复] Symbol：调整对齐，使其即使在长名称下也不破坏表格 ---
            ...
            # 注意：抽屉内容要恢复 white-space: normal 允许换行
            drawer_html = f"""
