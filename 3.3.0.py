...
# 版本号：GEMINI-3.3.0-Refactor-Phase1
# 
# 更新日志 (v3.3.0):
# 1. 【工程重构】引入 OrderUtils 工具类，统一封装订单解析逻辑。
#    - 彻底解决 PTrade API 返回对象/字典格式不确定的问题。
#    - 统一处理 symbol 标准化、状态判断、买卖方向识别。
# 2. 【代码瘦身】重写 _recalc_pending_frozen, cancel_all_orders_by_symbol, patrol_and_correct_orders。
#    - 移除冗余的 getattr/get 判断，调用 OrderUtils.normalize() 一步到位。
# 3. 【核心继承】完整保留 v3.2.72 的所有业务逻辑（NaN修复、单笔封顶、无缩放因子、OpenOrders撤单）。
...
__version__ = 'GEMINI-3.3.0-Refactor-Phase1'
...
class OrderUtils:
    ...
    def normalize(order):
        ...
        返回字段:
            - entrust_no: str
            - raw_symbol: str (API原始代码, e.g. 123.XSHG)
            - std_symbol: str (策略标准代码, e.g. 123.SS)
            - status: str
            - amount: float (带符号)
            - price: float
            - entrust_bs: str ('1'=买, '2'=卖)
            - original: obj (原始引用)
        """
        data = {}
...
def place_limit_orders(context, symbol, state, ignore_cooldown=False, bypass_lock=False, ignore_entrust_nos=None):
    ...
    # 【v3.2.72 新增】: 下单前最终价格有效性检查 (防止 NaN)
    if not is_valid_price(buy_p) or not is_valid_price(sell_p):
        if getattr(context, 'enable_debug_log', False):
            info('[{}] ⚠️ 计算出的挂单价格无效 (B:{} S:{})，跳过本次下单。', dsym(context, symbol), buy_p, sell_p)
        return
...
def update_grid_spacing_final(context, symbol, state, curr_pos):
    ...
    if new_buy != state.get('buy_grid_spacing') or new_sell != state.get('sell_grid_spacing'):
        ...
        info('[{}]     网格动态调整. (pos={}) ATR({:.2%}) -> 基础间距({:.2%}) -> 最终:[买{:.2%},卖{:.2%}]',
             dsym(context, symbol), pos, (atr_pct if atr_pct is not None and not math.isnan(atr_pct) else 0.0), base_spacing, new_buy, new_sell)
